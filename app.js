...
async function submitEmpOrder(){
  ...
  try{
    await db.collection('employee_orders').add({
      ...
      pageId,pageName,
      products:_empOrderCart.map(i=>({id:i.id,name:i.name,price:i.price,qty:i.qty,...(i.color?{color:i.color}:{}),...(i.writing?{writing:i.writing}:{})})),
      customerPhone:phone,
...
async function saveEmpOrderEdit(){
  ...
  try{
    ...
    const updateData={
      products:_empEditCart.map(i=>({id:i.id||'',name:i.name,price:parseFloat(i.price||0),qty:i.qty||1,...(i.color?{color:i.color}:{}),...(i.writing?{writing:i.writing}:{})})),
...
async function loadRosemaryWallet(){
  ...
  try{
    const snap=await db.collection('operator_expenses').get();
    _rwExpenses=snap.docs.map(d=>({id:d.id,...d.data(),_fromExpenses:true}));
  }catch(e){_rwExpenses=[];}
